        ...
        if request.method == 'GET':
            ...
            for i in range(int(inv_id)):
                ...
                for result_object in application:
                    ...
                    print(int(clearing_balance_credit) >= 0 & int(cl_credit_old) >= 0 &
                          int(clearing_balance_debit) == 0 & int(cl_debit_old) == 0)
                    print(int(clearing_balance_credit) >= 0 & int(cl_debit_old) >= 0 &
                          int(clearing_balance_debit) == 0 & int(cl_credit_old) == 0)
                    print(int(clearing_balance_debit) >= 0 & int(cl_credit_old) >= 0 &
                          int(clearing_balance_credit) == 0 & int(cl_debit_old) == 0)

                    if (int(clearing_balance_credit) == 0 & int(clearing_balance_debit) >= 0 &
                        int(cl_debit_old) >= 0 & int(cl_credit_old) == 0):
                        print(1)
                        new_debit_balance = int(clearing_balance_debit) + int(cl_debit_old)
                        new_credit_balance = int(cl_credit_old)
                    elif int(clearing_balance_credit) >= 0:
                        if int(cl_credit_old) >= 0:
                            if int(clearing_balance_debit) == 0:
                                print(int(cl_debit_old) == 0)
                                if int(cl_debit_old) == 0:
                                    print(2)
